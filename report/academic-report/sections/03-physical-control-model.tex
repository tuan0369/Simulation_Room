...
\end{align}
On first use, previous error is initialized to current error, so the derivative is zero. Integral clamping bounds its contribution to $\pm1$. After allocation, \code{apply\_actuator\_feedback} converts granted flow to applied output $u_i^{app}=g_i/0.16$ and bleeds the integral when $u_i>u_i^{app}$:
\begin{equation}I_i\leftarrow\clip\!\left(I_i-\frac{(u_i-u_i^{app})\Delta t}{10K_i};-20,20\right),\qquad K_i>0.\end{equation}
This back-calculation-style feedback reduces windup under shared-actuator saturation; it does not itself guarantee fairness or eliminate a sustained full request under persistent heat load. PID fundamentals and anti-windup concerns are discussed in \citet{astrom2006advanced}.
...
The room runtime stores the exact pre-state sensible thermal cooling used by the room update and AHU calculation, and MQTT publishes that stored value as \code{thermal\_cooling\_power\_w}. AHU telemetry separately publishes fan electrical power, COP-derived cooling electrical power, total electrical power, and cumulative electrical energy. Thus the earlier pre/post-step recomputation discrepancy is removed and thermal versus electrical quantities are explicit; they remain uncalibrated simulation estimates rather than facility measurements.

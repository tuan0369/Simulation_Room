...
\label{eq:risk}
\end{align}
The artifact version is \code{fan-risk-logistic-v1}. Runtime bands are
\begin{equation}
...
\end{align}
The 63 false negatives are operationally material. Algebraically derived from the same stored counts (not separately stored) are
\begin{align}
...
\end{align}
Holdout positive prevalence is $155/480=0.3229$, so an always-negative classifier would score 0.6771 accuracy; model accuracy is 0.1271 higher. Accuracy alone is insufficient, and PR curves are particularly useful under unequal class prevalence \citep{saito2015pr}. An earlier audit calculation from the deterministic holdout obtained AUROC approximately 0.8449, average precision 0.7315, log loss 0.4471, and Brier score 0.1467. These are clearly \emph{audit-derived}: the project does not generate or store them, and the final notebook should reproduce them with curves and code. Brier score assesses probability error \citep{brier1950}; calibration must still be plotted and tested \citep{guo2017calibration}.

\section{Validity and deployment gaps}
One random holdout has no temporal or asset-group separation, repeated seeds, confidence intervals, external data, drift evaluation, missing-value behavior, or out-of-distribution (OOD) gate. Directly injected runtime wear can produce combinations unlike training rows. Thresholds are not selected from maintenance costs. The appropriate evidence upgrade is an executed notebook with EDA, asset/time trajectories, grouped or temporal validation, baselines, calibration, bootstrap intervals, threshold-cost analysis, missing/OOD behavior, and an explicit model card \citep{mitchell2019modelcards}. That notebook is named in the README but is absent at report build time.

...
& Activities/deliverables and accountable role & Current source, JSON model artifact/metrics, two-room MQTT views, report, figures, and tests. Accountable role category: teaching/demo owner; named person remains a team assignment. Submission closure still requires the notebook, current executive pitch, metadata, and repaired/removed guided scenarios.\\
& KPIs and gate evidence & Verified current facts: 75 tests pass; allocator satisfies $0\le g_i\le r_i$ and $\sum g_i\le F^{avail}$ in tested cases; stored holdout counts are 92 TP, 31 FP, 63 FN, 294 TN at cutoff 0.5. Gate~0 is \textbf{not yet submission-complete}: require the missing named deliverables, one scripted repeatable run, raw telemetry/test output, and zero unresolved critical demo defects.\\
& Stop/rollback, ROI, and HW2 & If the model, broker, or UI is unavailable, use documented individual commands or stop the demo; do not imply live confirmation. ROI remains editable illustrative arithmetic only. Supports R1--R8 partially/as stated; R9 notebook and R10 final pitch remain unmet.\\
\midrule

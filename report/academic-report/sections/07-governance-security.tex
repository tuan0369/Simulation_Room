...
The local Mosquitto configuration states \code{allow\_anonymous true}, listens on plaintext MQTT port 1883 and WebSockets port 9001, and has no ACL evidence. The simulator checks JSON object shape, known topics, and principal value types/ranges, queues commands for serialized mutation, publishes retained telemetry, and declares an LWT offline state. Validation is incomplete: setpoint and AHU numeric commands do not explicitly reject non-finite values, and a numeric time-scale is converted with \code{int()} before membership testing, so some non-integer inputs can be truncated into an allowed value. These are useful reliability/input controls but do not authenticate an actor or protect confidentiality/integrity. Operational-technology security requires defense in depth, segmentation, identity, monitoring, recovery, and safely engineered control behavior \citep{nistot2023}.
...
Audit repudiation & actor denies command & weak accountability & dashboard/demo traces only & tamper-evident centralized audit with actor, time, decision, acknowledgement\\
